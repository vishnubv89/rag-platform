(function(){"use strict";(function(){const d=document.currentScript||document.querySelector("script[data-token]");if(!d)return;const I=new URL(d.src).origin,t={token:d.getAttribute("data-token")||"",orgId:parseInt(d.getAttribute("data-org")||"0",10),title:d.getAttribute("data-title")||"Ask AI",position:d.getAttribute("data-position")||"bottom-right",accentColor:d.getAttribute("data-accent-color")||"#D85A30",welcomeMessage:d.getAttribute("data-welcome")||"Ask me anything about your knowledge base",context:d.getAttribute("data-context")||"",apiUrl:I};if(!t.token){console.warn("[rag-widget] data-token is required");return}const U=Math.random().toString(36).slice(2),x=[];fetch(`${t.apiUrl}/widget/config`,{headers:{"X-Embed-Token":t.token}}).then(e=>e.ok?e.json():null).then(e=>{e&&(e.chatbot_name&&(t.title=e.chatbot_name),e.accent_color&&(t.accentColor=e.accent_color),e.position&&(t.position=e.position),e.welcome_message&&(t.welcomeMessage=e.welcome_message),O())}).catch(()=>{});const M=`
  ...
  `,w=document.createElement("div");w.setAttribute("id","rag-chat-widget"),document.body.appendChild(w);const c=w.attachShadow({mode:"open"}),v=document.createElement("style");v.textContent=M,c.appendChild(v);const T=t.position.includes("left")?" pos-left":"",b=document.createElement("button");b.className="bubble"+T,b.setAttribute("aria-label","Open chat"),b.innerHTML=`
    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/>
    </svg>`,c.appendChild(b);const _=e=>e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),r=document.createElement("div");r.className="panel hidden"+T,r.setAttribute("role","dialog"),r.setAttribute("aria-label",t.title),r.innerHTML=`
    <div class="panel-header">
      <span class="title" id="widget-title">${_(t.title)}</span>
    ...
    <div class="messages" id="msg-list">
      <div class="empty-state">
        ...
        <span id="widget-welcome">${_(t.welcomeMessage)}</span>
      </div>
    </div>
    <div class="input-row">
  ...
  `,c.appendChild(r);const p=c.getElementById("msg-list"),l=c.getElementById("chat-input"),A=c.getElementById("send-btn"),y=c.getElementById("mic-btn");function O(){v.textContent=M.replace(/--accent:\s*[^;]+;/,`--accent: ${t.accentColor};`);const e=t.position.includes("left");b.classList.toggle("pos-left",e),r.classList.toggle("pos-left",e),r.setAttribute("aria-label",t.title);const i=c.getElementById("widget-title");i&&(i.textContent=t.title);const n=c.getElementById("widget-welcome");n&&C&&(n.textContent=t.welcomeMessage)}let E=!1,C=!0;function S(e,i){C&&(p.innerHTML="",C=!1);const n=document.createElement("div");return n.className=`msg ${e}`,n.textContent=i,p.appendChild(n),p.scrollTop=p.scrollHeight,n}function B(e){E=e,A.disabled=e,l.disabled=e,y.disabled=e||k==="transcribing"}function P(e){return e.replace(/```[\w-]*\n?([\s\S]*?)```/g,"$1").replace(/`([^`]+)`/g,"$1").replace(/!\[([^\]]*)\]\([^)]+\)/g,"$1").replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").replace(/\*\*\*([^*]+)\*\*\*/g,"$1").replace(/___([^_]+)___/g,"$1").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/__([^_]+)__/g,"$1").replace(/\*([^*]+)\*/g,"$1").replace(/_([^_]+)_/g,"$1").replace(/~~([^~]+)~~/g,"$1").replace(/^#{1,6}\s+/gm,"").replace(/^>\s?/gm,"").replace(/^\s*[-*+]\s+/gm,"").replace(/^\s*\d+\.\s+/gm,"").replace(/[*_`#]/g,"").trim()}let f=null;function N(e,i){if(!i.trim())return;const n=document.createElement("div");n.className="msg-actions";const o=document.createElement("button");o.className="speak-btn",o.type="button",o.title="Play aloud",o.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>',o.addEventListener("click",async()=>{if(f&&!f.paused){f.pause(),f=null,o.classList.remove("playing");return}o.disabled=!0;try{const s=await fetch(`${t.apiUrl}/voice/speak`,{method:"POST",headers:{"Content-Type":"application/json","X-Embed-Token":t.token},body:JSON.stringify({text:P(i)})});if(!s.ok)throw new Error(`HTTP ${s.status}`);const g=await s.blob(),a=new Audio(URL.createObjectURL(g));f=a,o.classList.add("playing"),a.onended=()=>o.classList.remove("playing"),a.onerror=()=>o.classList.remove("playing"),await a.play()}catch(s){console.error("[rag-widget] speak failed",s)}finally{o.disabled=!1}}),n.appendChild(o),e.insertAdjacentElement("afterend",n),p.scrollTop=p.scrollHeight}let h=null,L=[],k="idle";function m(e){k=e,y.classList.toggle("recording",e==="recording"),y.disabled=e==="transcribing"||E}async function D(){var e;if(!((e=navigator.mediaDevices)!=null&&e.getUserMedia)){console.warn("[rag-widget] voice input not supported in this browser");return}try{const i=await navigator.mediaDevices.getUserMedia({audio:!0});L=[];const n=new MediaRecorder(i);h=n,n.ondataavailable=o=>{o.data.size>0&&L.push(o.data)},n.onstop=async()=>{i.getTracks().forEach(s=>s.stop());const o=new Blob(L,{type:"audio/webm"});if(o.size===0){m("idle");return}m("transcribing");try{const s=new FormData;s.append("file",o,"speech.webm");const g=await fetch(`${t.apiUrl}/voice/transcribe`,{method:"POST",headers:{"X-Embed-Token":t.token},body:s});if(!g.ok)throw new Error(`HTTP ${g.status}`);const{text:a}=await g.json();a!=null&&a.trim()&&(l.value=l.value?`${l.value} ${a.trim()}`:a.trim(),l.focus())}catch(s){console.error("[rag-widget] transcribe failed",s)}finally{m("idle")}},n.start(),m("recording")}catch{console.warn("[rag-widget] microphone access denied"),m("idle")}}function R(){(h==null?void 0:h.state)==="recording"&&h.stop()}y.addEventListener("click",()=>{k==="recording"?R():k==="idle"&&D()});async function z(){const e=l.value.trim();if(!e||E)return;l.value="",S("user",e),x.push({role:"user",content:e});const i=S("assistant","▌");i.classList.add("typing"),B(!0);let n="";try{const o={message:e,org_id:t.orgId,history:x.slice(-10),session_id:U};t.context&&x.length===1&&(o.message=`[Context: ${t.context}]

${e}`);const s=await fetch(`${t.apiUrl}/chat/stream`,{method:"POST",headers:{"Content-Type":"application/json","X-Embed-Token":t.token},body:JSON.stringify(o)});if(!s.ok)throw new Error(`HTTP ${s.status}`);const g=s.body.getReader(),a=new TextDecoder;let $="";for(;;){const{done:q,value:F}=await g.read();if(q)break;$+=a.decode(F,{stream:!0});const j=$.split(`
`);$=j.pop();for(const H of j)if(H.startsWith("data: "))try{const u=JSON.parse(H.slice(6));if(u.type==="token")n+=u.content,i.textContent=n+"▌",p.scrollTop=p.scrollHeight;else if(u.type==="done")i.textContent=n||u.answer||"",i.classList.remove("typing"),x.push({role:"assistant",content:n}),N(i,n||u.answer||"");else if(u.type==="error")throw new Error(u.message)}catch{}}}catch(o){i.textContent="⚠ Something went wrong. Please try again.",i.classList.remove("typing"),console.error("[rag-widget]",o)}finally{i.classList.contains("typing")&&(i.textContent=n||"(no response)",i.classList.remove("typing")),B(!1),l.focus()}}b.addEventListener("click",()=>{const e=!r.classList.contains("hidden");r.classList.toggle("hidden",e),e||l.focus()}),c.querySelector(".close").addEventListener("click",()=>{r.classList.add("hidden")}),A.addEventListener("click",z),l.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),z())}),document.addEventListener("click",e=>{w.contains(e.target)||r.classList.add("hidden")})})()})();
